...
이 문서는 현재까지 만든 내용을 단계 정리용 기준 문서로 남긴다. 이 프로젝트는 단순히 도시 안전 관제 대시보드를 만드는 것이 아니라, 관제 시스템이라는 주제 안에 학습한 프론트엔드 기술을 실제 화면과 코드 구조로 증명하는 것이 목적이다.
...
면접관이 화면을 봤을 때 단순히 "대시보드를 만들었다"에서 끝나면 약하다. 그래서 화면 위에 X-Ray 라벨을 띄워서 어떤 UI가 어느 FSD 계층, 어느 패키지, 어느 기술스택에서 왔는지 직접 볼 수 있게 한다.
...
이 프로젝트는 기능 구현물인 동시에 학습 기록물이다. 그래서 코드와 문서가 같이 가야 한다.
