...
현재 selector에서 전체, FSD-style, Module Federation, Monorepo, OpenLayers 관점을 선택한다. FSD-style은 일반 UI 책임 경계를, Module Federation은 runtime remote 경계를, Monorepo는 workspace 연결을, OpenLayers는 지도 렌더링 경계를 표시한다.
...
현재 구현된 관점은 `architecture`의 FSD-style·Monorepo, `technology`의 OpenLayers, `delivery`의 Module Federation 표시다. 나머지 관점은 해당 기술의 실제 연결과 함께 추가한다.
