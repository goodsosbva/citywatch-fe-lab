...
홈 대시보드가 보유한 사고 데이터를 별도 Vite 앱의 분석 함수로 전달하고, 계산 결과를 다시 화면에 표시하는 Module Federation 흐름을 구현했다.

```txt
Next.js host: http://127.0.0.1:3000
Vite remote:  http://127.0.0.1:3002
Manifest:     http://127.0.0.1:3002/mf-manifest.json
Expose:       citywatch_analytics/incident-analytics
```

홈 화면의 `원격 사고 분석` 영역은 분석 대상 수, 고위험 사고 수, 해결률, 평균 영향 인원을 보여준다. X-Ray를 켜면 이 계산이 로컬 번들 코드가 아니라 `apps/analytics-remote`에서 왔다는 경계를 확인할 수 있다.
...
실제 공개 모듈은 `apps/analytics-remote/src/incident-analytics.ts`다.

```ts
export function calculateIncidentAnalytics(incidents: Incident[]) {
  // 전체 수, 고위험 수, 해결률, 평균 영향 인원을 계산한다.
}
```

입력은 `packages/api-types`의 `Incident[]`를 사용한다. 같은 도메인 계약을 사용하므로 host가 가진 사고 데이터와 remote가 기대하는 입력 구조가 TypeScript 수준에서 어긋나지 않는다.

계산 코드를 순수 함수로 만든 이유는 네트워크, React, 브라우저 상태 없이 입력과 출력만으로 검증하기 위해서다. 테스트는 `apps/analytics-remote/src/incident-analytics.test.ts`에 있으며 일반 데이터와 빈 배열을 확인한다.
...
→ loadRemote("citywatch_analytics/incident-analytics")
→ export 형태 검증
→ calculateIncidentAnalytics(incidents)
→ 결과 형태 검증
→ 화면 상태 갱신
```
...
TypeScript 타입은 개발 중 실수를 막지만 네트워크에서 받은 모듈의 실제 형태까지 보장하지 않는다. 따라서 host는 함수 export와 계산 결과의 숫자 필드를 런타임에도 확인한다. remote가 잘못 배포되거나 계약이 깨졌을 때 잘못된 값을 조용히 렌더링하지 않기 위해 필요한 경계 검증이다.
...
→ remote 분석 함수 호출
→ 분석 결과를 host의 metric UI로 렌더링
```

remote가 담당하는 것은 계산이고 UI 렌더링은 host가 담당한다. remote 로드 중에는 상태 메시지를, 실패하면 `role="alert"` 오류와 재시도 버튼을 표시한다. 따라서 remote 서버가 일시적으로 내려가도 홈 전체가 깨지지 않는다.
...
홈에서 `원격 사고 분석`과 `Federated` 표시가 나오면 로드가 성공한 것이다. X-Ray를 켜서 `remote/CalculateIncidentAnalytics` 경계도 확인한다.
