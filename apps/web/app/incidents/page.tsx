    ...
    >
      <section className="panel" aria-labelledby="incident-filter-title">
        ...
        <p className="redux-proof" aria-live="polite">
          <span>{restApiXray ? "REST 조회 조건" : "Redux 필터"} {activeFilterCount}개 적용</span>
...
function RestApiEvidencePanel({
  ...
  return (
    <aside aria-labelledby="rest-api-evidence-title" className="panel technology-evidence">
      <div className="panel-title-row">
        <h2 id="rest-api-evidence-title">REST API 실행 증거</h2>
        <Badge tone={error ? "danger" : loading ? "info" : "success"}>
          {error ? "응답 오류" : loading ? "요청 중" : `${incidentCount}건 검증 완료`}
        </Badge>
      </div>

      <p>
        필터 선택으로 조회 조건이 바뀌면 브라우저가 실제 GET 요청을 보내고, Route Handler가 검증한 query로 저장소를 조회합니다. 브라우저는 응답 계약을 다시 검사한 뒤에만 React 목록 상태를 바꿉니다.
      </p>
      ...
      <dl className="technology-code">
        <div>
          <dt>현재 실제 요청</dt>
          <dd><code>GET {requestUrl}</code></dd>
        </div>
        <div>
          <dt>Route Handler → store</dt>
          <dd><code>GET → listIncidents(query)</code></dd>
        </div>
        <div>
          <dt>현재 요청 상태</dt>
          <dd><code>{error ? `error: ${error}` : loading ? "fetch pending" : `validated response: ${incidentCount} incidents`}</code></dd>
        </div>
        <div>
          <dt>공유 계약</dt>
          <dd><code>{JSON.stringify(query)} → IncidentListResponse</code></dd>
        </div>
      </dl>
    </aside>
  );
